sudo tee /opt/elk/logstash/pipeline/wizard-spider.conf << 'PIPELINE'
input {
  beats {
    port => 5044
  }
...
filter {
  ...
  # ---- Normalize Sysmon event types (generic, no attack context) ----
  ...
  # ---- Enrich with generic network direction (outbound vs inbound) ----
  if [event_type] == "network_connection" {
    if [winlog][event_data][Initiated] == "true" {
      mutate { add_field => { "network_direction" => "outbound" } }
    } else {
      mutate { add_field => { "network_direction" => "inbound" } }
    }
  }

  # ---- Copy key fields to top level for easier querying ----
  if [winlog][event_data][Image] {
    mutate { add_field => { "process_image" => "%{[winlog][event_data][Image]}" } }
  }
  if [winlog][event_data][ParentImage] {
    mutate { add_field => { "parent_image" => "%{[winlog][event_data][ParentImage]}" } }
  }
  if [winlog][event_data][CommandLine] {
    mutate { add_field => { "command_line" => "%{[winlog][event_data][CommandLine]}" } }
  }
  if [winlog][event_data][User] {
    mutate { add_field => { "event_user" => "%{[winlog][event_data][User]}" } }
  }
  if [winlog][event_data][TargetFilename] {
    mutate { add_field => { "target_file" => "%{[winlog][event_data][TargetFilename]}" } }
  }
  if [winlog][event_data][TargetObject] {
    mutate { add_field => { "registry_target" => "%{[winlog][event_data][TargetObject]}" } }
  }
  if [winlog][event_data][DestinationIp] {
    mutate { add_field => { "dst_ip" => "%{[winlog][event_data][DestinationIp]}" } }
  }
  if [winlog][event_data][DestinationPort] {
    mutate { add_field => { "dst_port" => "%{[winlog][event_data][DestinationPort]}" } }
  }
  if [winlog][event_data][SourceIp] {
    mutate { add_field => { "src_ip" => "%{[winlog][event_data][SourceIp]}" } }
  }
  if [winlog][event_data][SourcePort] {
    mutate { add_field => { "src_port" => "%{[winlog][event_data][SourcePort]}" } }
  }
}
